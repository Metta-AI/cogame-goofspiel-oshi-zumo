...
# Substitute: goofspiel-oshi-zumo, coworld-goofspiel-oshi-zumo, 4.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-goofspiel-oshi-zumo:ci)
#   SMOKE_SLUG                 game slug                        (goofspiel-oshi-zumo)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/goofspiel-oshi-zumo)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/goofspiel-oshi-zumo-player)
...
image="${1:-${SMOKE_IMAGE:-coworld-goofspiel-oshi-zumo:ci}}"
slug="${SMOKE_SLUG:-goofspiel-oshi-zumo}"
...
def validate(instance, schema, path="results"):
    """Enough of JSON Schema to hold the game to its own results_schema.

    Not a general validator: required/type/enum/minimum/maximum/minItems/
    maxItems/additionalProperties over the shape this manifest declares.
    A results.json that does not match its schema is a coworld whose league
    rows are unreadable, and nothing else in CI looks at it.
    """
    problems = []
    kinds = {
    ...
    if want:
        expect = kinds[want]
        ok = isinstance(instance, expect) and not (
            want in ("number", "integer") and isinstance(instance, bool))
        if not ok:
        ...
        if schema.get("additionalProperties") is False:
            for key in instance:
                if key not in properties:
                    problems.append(f"{path}.{key}: not in the schema")
        for key, sub in properties.items():
            if key in instance:
                problems += validate(instance[key], sub, f"{path}.{key}")
    elif want == "array":
        if "minItems" in schema and len(instance) < schema["minItems"]:
            problems.append(f"{path}: {len(instance)} items < minItems")
        if "maxItems" in schema and len(instance) > schema["maxItems"]:
            problems.append(f"{path}: {len(instance)} items > maxItems")
        if "items" in schema:
            for index, item in enumerate(instance):
                problems += validate(item, schema["items"], f"{path}[{index}]")
    else:
        if "enum" in schema and instance not in schema["enum"]:
            problems.append(f"{path}: {instance!r} not in {schema['enum']}")
        if "minimum" in schema and instance < schema["minimum"]:
            problems.append(f"{path}: {instance!r} < {schema['minimum']}")
        if "maximum" in schema and instance > schema["maximum"]:
            problems.append(f"{path}: {instance!r} > {schema['maximum']}")
    return problems
...
if schema:
    failures = validate(results, schema)
    if failures:
        raise SystemExit("results.json does not match results_schema:\n  " +
                         "\n  ".join(failures))
    print("results.json validates against game.results_schema")
else:
    print("WARNING: the manifest declares no game.results_schema")
