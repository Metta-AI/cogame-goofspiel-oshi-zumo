// Goofspiel / Oshi-Zumo static replay shell: fetches the replay named by ?replay=<url>,
...
(function () {
  ...
  function start(module, bytes) {
    ...
    var ok = module._gzu_load_replay(ptr, bytes.length);
    module._free(ptr);
    if (!ok) {
      fail(readString(module, module._gzu_error_ptr(),
        module._gzu_error_len()) || "wasm rejected the replay");
      return;
    }
    var payload = JSON.parse(
      readString(module, module._gzu_payload_ptr(),
        module._gzu_payload_len())
    );
    var loading = document.getElementById("loading");
    if (loading) loading.style.display = "none";
    document.documentElement.removeAttribute("data-replay-error");
    GozuRenderer.attachReplay({
      ...
      assetBase: "./assets",
      payload: payload,
      // ONE deviation from the cogame-babel shell, and the reason for it:
      // babel set data-replay-loaded inside the renderer's ready callback
      // but posted the bridge `ready` from a double-rAF at the CALL SITE, so
      // `ready` could precede the first painted frame and an embedding page
      // could sample an unpainted shell (chorus 3c11c953, 2026-08-24).
      // attachReplay sets the attribute on its first DRAWN frame and then
      // calls this back, so `ready` can only ever follow a picture.
  ...
  function load() {
    ...
    caption(attempt > 1 ? "RETRYING REPLAY… (attempt " + attempt + ")" :
      "LOADING REPLAY…");
    if (!modulePromise) {
      modulePromise = GozuReplayModule().catch(function (error) {
